...
mod holds_and_no_ops {
    ...
    /// it, so one reached without the shape is neither restyled nor laid out again: grown
    /// and stamped, the shape went to the server and to them. Nothing reaches it: a press
    /// on a label is a press on its shape (`App.tsx@1118751f:6784-6829`), and the
    /// selection lets in neither the shape nor its label — selected on its own by the host,
    /// or by Select All (`peers.rs` › `is_held`).
    #[test]
    fn a_shape_a_peer_holds_is_not_grown_through_its_label() {
        ...
        assert!(engine.get_selection().is_empty(), "the label on its own");
        engine.set_font_size(80.0);
        engine.set_font_family(7);
        engine.paste_styles();
        engine.select_all();
