...
//!   `charWidth` caches them, plus a bounded memo of wrapped hard lines keyed by
//!   (line, width, [`FontKey`]), so a keystroke re-wraps only the line it touched.
//!
...
pub use measure::{CachedMetrics, FontKey, MeasureCache, TextMetrics, MEMO_LIMIT};
